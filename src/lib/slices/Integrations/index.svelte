...
>
	...
	<div class="relative">
		...
		<div class="mt-2 flex flex-col items-center md:flex-row">
			{#each slice.primary.groupicons as item, index}
				{#if index === Math.floor(slice.primary.groupicons.length / 2)}
					<StylizedLogoMark />
				...
				{#if item.icon}
					...
					{#if index !== slice.primary.groupicons.length - 1}
						<div
							class={clsx(
								'signal-line',
								index >= Math.floor(slice.primary.groupicons.length / 2) ? 'rotate-180' : 'rotate-0'
							)}
						></div>
					{/if}
				{/if}
