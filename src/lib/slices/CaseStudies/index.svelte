			...
			>
				<div class="col-span-1 flex flex-col justify-center gap-4">
					...
					<PrismicLink
						...
						>Read <PrismicText field={caseStudy.data.company} /> case study</PrismicLink
					>
				</div>
				...
				<!-- <PrismicLink field={item.case_study}>Link</PrismicLink> -->
			</div>
		{/each}
	</div>
</Bounded>
...
<style>
	.image-glow {
		@apply absolute h-1/2 w-1/2 rounded-full opacity-40 mix-blend-screen blur-3xl transition-opacity duration-500 group-hover:opacity-30;
	}
</style>
